...
export async function getProgrammingLanguageReport(language: string): Promise<CandidateReport[]> {
  ...
  const submissionMap = new Map<string, { submittedAt: Date; user: any }>();
  ...
  const report: CandidateReport[] = Array.from(submissionMap.values()).map((data) => {
    return {
      name: data.user.name,
...
export async function getDepartmentReport(department: string): Promise<CandidateReport[]> {
  ...
  // Create a map of userId -> submission data
  const submissionMap = new Map<string, { submittedAt: Date }>();
  submissions.forEach((sub) => {
  ...
  // Build report
  const report: CandidateReport[] = candidates.map((candidate) => {
